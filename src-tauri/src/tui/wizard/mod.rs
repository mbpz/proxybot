            ...
            Platform::IOS => vec![
                WizardStep {
                    ...
                    instruction: "Use AirDrop to send ca.crt to your iPhone/iPad.\n\
                                 Or download the certificate file to your device."
                        .to_string(),
                    deep_link: None,
                },
                WizardStep {
                ...
                WizardStep {
                    ...
                    instruction: "Go to Settings > General > About >\n\
                                 ...
                                 Enable full trust for \"ProxyBot CA\"."
                        .to_string(),
                    deep_link: Some(
                        "App-prefs:root=General&path=About/CERTIFICATE_TRUST_SETTINGS".to_string(),
                    ),
                },
                WizardStep {
            ...
            Platform::Android => vec![
                ...
                WizardStep {
                    ...
                    instruction: "Open the ca.crt file in Files app.\n\
                                 You may need to locate it in Downloads."
                        .to_string(),
                    deep_link: None,
                },
                WizardStep {
                ...
                WizardStep {
                    ...
                    instruction: "On Android 7+, user-added certificates don't\n\
                                 ...
                                 VPN mode for full traffic capture."
                        .to_string(),
                    deep_link: None,
                },
            ],
...
pub fn render_wizard(f: &mut Frame, area: ratatui::layout::Rect, wizard: &CertWizard) {
    ...
    // Platform indicator
    let platform_indicator =
        Paragraph::new(format!("Platform: {}", platform_name)).style(Color::Yellow);
    ...
    if wizard.current_step < wizard.steps.len() {
        ...
        let step_title = Paragraph::new(step.title.clone()).style(Color::Green);
        f.render_widget(step_title, chunks[1]);

        let instruction_lines: Vec<Line> = step
            .instruction
            .lines()
        ...
        if let Some(ref link) = step.deep_link {
            ...
            let link_para = Paragraph::new(link_text).style(Color::Blue);
            f.render_widget(link_para, chunks[2]);
        }
    }

    // Progress indicator
    let progress = format!("Step {}/{}", wizard.current_step + 1, wizard.steps.len());
    let progress_para = Paragraph::new(progress).style(Color::White);
    ...
    let hints_para = Paragraph::new(hints).style(Color::Gray);
...
mod tests {
    ...
    fn test_platform_detection_ios() {
        assert_eq!(
            Platform::detect_from_ua("Mozilla/5.0 (iPhone; CPU iPhone OS 15_0)"),
            Platform::IOS
        );
        assert_eq!(
            Platform::detect_from_ua("Mozilla/5.0 (iPad; CPU OS 14_0)"),
            Platform::IOS
        );
    }
    ...
    fn test_platform_detection_android() {
        assert_eq!(
            Platform::detect_from_ua("Mozilla/5.0 (Linux; Android 11)"),
            Platform::Android
        );
    }
    ...
    fn test_platform_detection_unknown() {
        assert_eq!(
            Platform::detect_from_ua("Mozilla/5.0 (Windows NT 10.0)"),
            Platform::Unknown
        );
    }
