...
pub fn render(f: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    lines.push(format!("┌─ {} ─────────────────────────────────────────────────┐", generator_label));
    lines.push("│                                                         │".to_string());

    // Mode selector
    lines.push(format!("│  {}                                                  │", mode_label));
    ...
    lines.push("│                                                         │".to_string());
    lines.push(format!("│  {}                                              │", tr(K::GenActions)));
    ...
    lines.push(format!("│  {}                                               │", tr(K::GenOutput)));

    if app.gen.is_generating {
        lines.push("│  ┌─────────────────────────────────────────────────┐  │".to_string());
        lines.push(format!("│  │ {}                       │  │", tr(K::GenGenerating)));
        lines.push("│  └─────────────────────────────────────────────────┘  │".to_string());
    } else if app.gen.progress_output.is_empty() {
        lines.push("│  ┌─────────────────────────────────────────────────┐  │".to_string());
        lines.push(format!("│  │ {}     │  │", tr(K::GenNoGenerationYet)));
        lines.push(format!("│  │ {}             │  │", tr(K::GenSelectModeAndPress)));
        lines.push("│  └─────────────────────────────────────────────────┘  │".to_string());
    } else {
    ...
    if let Some(ref path) = app.gen.output_path {
        lines.push(format!("│  {}                                          │", tr(K::GenOutputLast)));
    ...
    lines.push(format!("│        {}                  │", tr(K::GenRunInference)));
    lines.push("└─────────────────────────────────────────────────────────┘".to_string());

    let content = lines.join("\n");
    let gen_title = tr(K::GenTitle);
    let para = Paragraph::new(content)
        .block(Block::default().borders(Borders::ALL).title(format!("{} │ {}", gen_title, tr(K::GenMode))));
