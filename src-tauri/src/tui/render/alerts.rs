...
fn render_header(f: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    let new_domain_alerts = alerts
        .iter()
        .filter(|a| matches!(a.alert_type, crate::anomaly::AlertType::NewDomain) && !a.acknowledged)
        .count();
    ...
    let para = Paragraph::new(header_text)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(alerts_summary_title.as_str()),
        )
        .style(Color::White);
...
fn render_alert_list(f: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    if alerts.is_empty() {
        let alerts_title = tr(K::AlertsTitle);
        let empty = Paragraph::new(format!("  {}", tr(K::AlertsEmpty))).block(
            Block::default()
                .borders(Borders::ALL)
                .title(alerts_title.as_str()),
        );
    ...
    let rows: Vec<Row> = alerts
        ...
        .map(|(idx, alert)| {
            ...
            let row = Row::new(vec![severity_cell, ts_cell, desc_cell, type_cell]).style(
                if idx == selected {
