...
fn test_filter_bar_shows_active_method_filter() {
    ...
    assert!(
        line.contains("[GET]"),
        "active method filter should show [GET]"
    );
}
...
fn test_filter_bar_shows_host_filter() {
    ...
    assert!(
        line.contains("Host:[api"),
        "host filter should show api pattern"
    );
}
...
fn test_filter_bar_shows_search_input() {
    ...
    assert!(
        line.contains("/api/"),
        "search input should be displayed as /api/"
    );
}
...
fn test_filter_bar_truncates_long_host() {
    ...
    assert!(
        line.contains("this-is-a-very"),
        "long host should be truncated to 15 chars"
    );
}
...
fn test_filter_bar_shows_empty_app_tag() {
    ...
    assert!(
        line.contains("App:["),
        "empty app tag should show as App:[]"
    );
}
...
fn test_filter_bar_controls_key_hints() {
    ...
    assert!(
        controls.contains("Enter"),
        "controls should show Enter hint"
    );
    assert!(
        controls.contains("search"),
        "controls should show search hint"
    );
    assert!(
        controls.contains("detail"),
        "controls should show detail hint"
    );
}
