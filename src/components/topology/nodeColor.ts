import { NodeKind, TopologyNode } from "./types";

const COLORS = {
  device: { background: "rgba(0,212,255,0.15)", border: "#00d4ff", text: "#00d4ff" },
  app: { background: "rgba(34,197,94,0.12)", border: "#22c55e", text: "#22c55e" },
  host: { background: "rgba(136,136,170,0.1)", border: "#1e1e2e", text: "#ffffff" },
  proxy: { background: "rgba(168,85,247,0.15)", border: "#a855f7", text: "#a855f7" },
};
...
export function nodeTextColor(node: TopologyNode | NodeKind): string {
  const kind: NodeKind = typeof node === "string" ? node : node.kind;
